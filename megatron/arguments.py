...
def validate_args(args, defaults={}):
    ...
    args.pipeline_model_parallel_size = min(
        args.pipeline_model_parallel_size, (args.world_size // args.tensor_model_parallel_size)
    )
    ...
    if args.pipeline_model_parallel_size > 1:
        if args.pipeline_model_parallel_split_rank is not None:
            assert args.pipeline_model_parallel_split_rank < args.pipeline_model_parallel_size, (
                'split rank needs'
    ...
    if args.overlap_param_gather:
        ...
        assert args.use_mcore_models, '--overlap-param-gather only supported with MCore models'
    ...
    if args.retro_add_retriever:
        ...
        assert not args.sequence_parallel, "retro currently does not support sequence parallelism."
    ...
    if args.rotary_interleaved and not args.use_mcore_models:
        raise RuntimeError(
            '--rotary-interleaved only support Megatron Core, please add --use-mcore-models.'
        )
    ...
    if args.expert_model_parallel_size > 1:
        ...
        assert not args.fp16, "Expert parallelism is not supported with fp16 training."

    # Distributed checkpointing checks
    if args.use_dist_ckpt and not args.use_mcore_models:
        raise RuntimeError(
            '--use-dist-ckpt only support Megatron Core, please add --use-mcore-models.'
        )
...
def _add_transformer_engine_args(parser):
    ...
    group.add_argument(
        '--fp8-margin', type=int, default=0, help='Scaling margin for fp8', dest='fp8_margin'
    )
    group.add_argument(
        '--fp8-interval',
        type=int,
...
def _add_retro_args(parser):
    ...
    group.add_argument(
        "--retro-attention-gate", type=float, default=1, help="Gated cross attention."
    )
    group.add_argument(
...
def _add_network_size_args(parser):
    ...
    group.add_argument(
        '--rotary-interleaved', action='store_true', help='Use interleaved rotary embedding.'
    )
    group.add_argument(
    ...
    group.add_argument(
        '--norm-epsilon', type=float, default=1e-5, help='Epsilon for layer norm and RMS norm.'
    )
    group.add_argument(
        '--apply-layernorm-1p',
        action='store_true',
...
def _add_logging_args(parser):
    ...
    group.add_argument(
        '--log-params-norm', action='store_true', help='If set, calculate and log parameters norm.'
    )
    group.add_argument(
    ...
    group.add_argument(
        '--tensorboard-log-interval', type=int, default=1, help='Report to tensorboard interval.'
    )
    group.add_argument(
    ...
    group.add_argument(
        '--wandb-save-dir', type=str, default='', help='Path to save the wandb results locally.'
    )
    group.add_argument(
        '--enable-one-logger',
        action='store_true',
    ...
    group.add_argument(
        '--one-logger-project',
        type=str,
        default='e2e-tracking',
        help='The one-logger project name. Will ignore if ' '--enable-one-logger is not set',
    )
    group.add_argument(
        '--one-logger-entity',
        type=str,
    ...
    group.add_argument(
        ...
        type=str,
        default=None,
        help='The one-logger run name displayed. Will ignore if ' '--enable-one-logger is not set',
    )
    return parser
...
def _add_regularization_args(parser):
    ...
    group.add_argument(
        '--attention-dropout', type=float, default=0.1, help='Post attention dropout probability.'
    )
    group.add_argument(
        '--hidden-dropout',
        type=float,
...
def _add_training_args(parser):
    ...
    group.add_argument(
        '--profile-step-start', type=int, default=10, help='Global step to start profiling.'
    )
    group.add_argument(
        '--profile-step-end', type=int, default=12, help='Global step to stop profiling.'
    )
    group.add_argument(
    ...
    group.add_argument(
        '--validation_drop_last', action='store_false', help='Drop last partial validation samples.'
    )
    group.add_argument(
        '--pad_to_global_batch_size',
        action='store_false',
    ...
    group.add_argument(
        '--optimizer', type=str, default='adam', choices=['adam', 'sgd'], help='Optimizer function'
    )
    group.add_argument(
        '--dataloader-type',
        type=str,
    ...
    group.add_argument(
        '--sequence-parallel', action='store_true', help='Enable sequence parallel optimization.'
    )
    group.add_argument(
...
def _add_checkpointing_args(parser):
    ...
    group.add_argument(
        '--save', type=str, default=None, help='Output directory to save checkpoints to.'
    )
    group.add_argument(
        '--save-interval',
        type=int,
    ...
    group.add_argument(
        '--load', type=str, default=None, help='Directory containing a model checkpoint.'
    )
    group.add_argument(
        '--no-load-optim',
        action='store_true',
    ...
    group.add_argument(
        '--use-dist-ckpt', action='store_true', help='Use distributed checkpoint format.'
    )
    group.add_argument(
...
def _add_mixed_precision_args(parser):
    ...
    group.add_argument(
        '--fp32-residual-connection', action='store_true', help='Move residual connections to fp32.'
    )
    group.add_argument(
        '--apply-query-key-layer-scaling',
        action='store_true',
        help='Scale Q * K^T by 1 / layer-number. ' 'Useful for fp16 training.',
    )
    group.add_argument(
...
def _add_distributed_args(parser):
    ...
    group.add_argument(
        ...
        action='store_true',
        default=False,
        help='If set, delay / synchronize param all-gathers in all but first PP stage.',
    )
    group.add_argument(
